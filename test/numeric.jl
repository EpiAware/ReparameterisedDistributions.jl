...
@testitem "Weibull(mean, sd): moment recovery sweep" begin
    ...
    @test Distributions.shape(
        native(
            reparameterise(
                Weibull;
                mean = 1.0, sd = 0.5227232008770631
            )
        )
    ) ≈ 2.0 atol = 1.0e-10
end
...
@testitem "A numeric family needs valid_moments and to_native" begin
    ...
    function ReparameterisedDistributions.to_native(
        ...
        m, sd = vals
        s = solve_moment(
            Gamma, Val((:mean, :sdnumeric)), _residual, _deriv,
            _bracket, vals
        )
        shape = exp(s)
    ...
    for cv in (0.05, 0.2, 1.0, 3.0, 5.0)
        ...
        @test native(d).α ≈ analytic.α rtol = 1.0e-10
        @test native(d).θ ≈ analytic.θ rtol = 1.0e-10
    end
end
...
@testitem "Weibull(mean, sd): the valid CV domain round-trips at its edges" begin
    ...
    for cv in (cv_min * 1.01, cv_max * 0.99)
        ...
        @test mean(d) ≈ 1.0 rtol = 1.0e-6
        @test std(d) ≈ cv rtol = 1.0e-6
    end
end
...
@testitem "Weibull(mean, sd): check_args = false gives -Inf, not a throw" begin
    ...
    cv_max = ReparameterisedDistributions._weibull_cv_max(Float64)
    d = reparameterise(
        Weibull; mean = 1.0, sd = cv_max * 1.5,
        check_args = false
    )
    ...
    d32 = reparameterise(
        Weibull; mean = 1.0f0, sd = Float32(cv_max) * 1.5f0,
        check_args = false
    )
