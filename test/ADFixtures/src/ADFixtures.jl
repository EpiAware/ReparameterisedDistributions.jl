...
module ADFixtures
    ...
    function _nbinom_loglik(θ, counts)
        d = reparameterise(
            NegativeBinomial; mean = θ[1], overdispersion = θ[2],
            check_args = false
        )
    ...
    function _nbinom_dispersion_loglik(θ, counts)
        d = reparameterise(
            NegativeBinomial; dispersion = θ[1], mean = θ[2],
            check_args = false
        )
        return sum(k -> logpdf(d, k), counts)
    end
    ...
    function _skewnormal_loglik(θ, obs)
        d = reparameterise(
            SkewNormal; centre = θ[1], mass_below_centre = θ[2],
    ...
    function _invgauss_meansd_loglik(θ, obs)
        d = reparameterise(
            InverseGaussian; mean = θ[1], sd = θ[2],
            check_args = false
        )
        return sum(x -> logpdf(d, x), obs)
    end
    ...
    function scenarios(; with_reference::Bool = false, category::Symbol = :marginal)
        ...
        cases = (
            ("LogNormal(mean, sd) loglik", _meansd_loglik, [8.0, 2.0], reals),
