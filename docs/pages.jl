# PACKAGE-OWNED — scaffold writes this once and never overwrites it.
# The docs navigation tree. Add tutorial / guide pages here as the package
# grows; `make.jl` reads `pages` from this file. The API reference and the
# README-derived home page are generated by `make.jl`.
#
# The Benchmarks entry below is written only when the package is scaffolded
# with `benchmarks = true` (the BENCHMARKS_NAV substitution); a package
# without benchmarks gets no entry and no page. With `ad = true` its
# "AD comparison" child carries the kit-managed AD-backend-comparison page.
#
# Every entry must resolve to a page the package writes or `make.jl` generates.
# Docs about the kit itself (customising the generated site, how template sync
# keeps a repo current) live on the kit's own site, not on a package's (#194).

pages = [
    "Home" => "index.md",
    # Authored quickstart (a package-owned source page, distinct from the
    # README-derived home page). Add tutorials under `getting-started/` as the
    # package grows.
    "Getting started" => [
        "Overview" => "getting-started/index.md",
        "Tutorials" => [
            "Priors on moments" => "getting-started/tutorials/priors-on-moments.md",
        ],
    ],
    # Developer-facing pages: the extension contract a new parameterisation
    # is registered against, and how a registration is tested — here or from
    # another package.
    "Developer" => [
        "Adding a reparameterisation" => "developer/adding-a-reparameterisation.md",
    ],
    "API reference" => [
        "Public API" => "lib/public.md",
        "Internal API" => "lib/internals.md",
    ],
    "Benchmarks" => [
        "Performance over time" => "benchmarks/over-time.md",
        "AD comparison" => "benchmarks/ad-comparison.md",
    ],
    "Release notes" => "release-notes.md",
]
