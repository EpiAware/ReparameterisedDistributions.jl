...
What each AD backend costs on ReparameterisedDistributions.jl's shared AD scenario set, so a
...
[README](https://github.com/EpiAware/ReparameterisedDistributions.jl#readme), which reports each backend's
